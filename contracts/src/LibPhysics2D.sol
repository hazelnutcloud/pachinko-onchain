...
library LibPhysics2D {
    ...
    int256 internal constant GRAVITY = 98e17; // 9.8 m/s^2
    ...
    function step(
        ...
        for (uint256 i = 0; i < bodyCount; i++) {
            ...
            for (uint256 j = 0; j < mapBodyCount; j++) {
                ...
                (RigidBody memory b1, , bool collided) = resolveCollision(
                    world.bodies[i],
                    map.bodies[j]
                );
    ...
    function getBody(
        World storage world,
        uint256 id
    ) internal view returns (RigidBody memory) {
        if (id >= world.bodies.length) revert BodyDoesNotExist();
